---
title: "App defaults in late 2023"
date: 2023-11-08
syndication:
  - https://mastodon.social/@barryf/111377054725317823
---

Following the recent [trend](https://defaults.rknight.me/) of posting your default apps, here are mine:

- 📨 Mail Client: Mimestream, Gmail app
- 📮 Mail Server: Google Workspace
- 📝 Notes: Reflect
- ✅ To-Do: Things, Reflect Tasks
- 🟦 Photo Management: iCloud Photos
- 📆 Calendar: iCloud Calendar
- 📁 Cloud File Storage: iCloud Drive
- 📖 RSS: Readwise Reader
- 🙍🏻‍♂️ Contacts: iCloud Contacts
- 🌐 Browser: Safari
- 💬 Chat: WhatsApp, Slack
- 🔖 Bookmarks: [/bookmarks](https://barryfrost.com/bookmarks), Pinboard
- 📑 Read It Later: Readwise Reader
- 📜 Word Processing: Google Docs, Pages
- 📈 Spreadsheets: Google Sheets
- 📊 Presentations: Google Slides, Keynote
- 🛒 Shopping Lists: Notes (iCloud sharing)
- 📰 News: The Guardian, BBC News
- 🎵 Music: Spotify
- 🎤 Podcasts: Spotify
- 🔐 Password Management: 1Password

/via [hacdias](https://hacdias.com/2023/11/08/app-defaults-late-2023/).
