...
Paginated pages show `Title (Page N)` in both the h1 and the browser window title. The `Pagination` component shows all page numbers (no window/cap). Page-1-only content (section intros, the check-ins map, the books Reading section) is gated on `page.currentPage === 1` — note that slotted content can't be wrapped in a conditional (Astro extracts slots at compile time), so intros are hidden via `Feed.astro`'s `showDescription` prop instead.
...
| `/books` | Books — "Reading" then "Finished", stacked, each a responsive grid |
...
- `BookCard` — clickable cover linking to BookHive, title, authors, "Started/Finished [relative date]", pdsls icon link. A `size` prop picks the cover width: `sm` (60px, the default, used by Finished) or `lg` (96px, used by Reading)
...
- `formatDateRelative(date, now?)` — relative time ("today", "yesterday", "3 days ago") for dates within 14 days; falls back to `formatDateShort` beyond that. Always whole Europe/London calendar days, never hours or minutes, even for timestamped values: the label is computed at build time and the site only rebuilds hourly, so "5 minutes ago" would sit there stale for up to an hour while "today" stays true all day. The exact time is still available in the `title` attribute via `formatDateTitle`. Used for every visible displayed date.
...
- Books — indexed per card (`data-pagefind-body` on `BookCard`), covering both the Reading and Finished lists
