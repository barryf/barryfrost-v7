...
| `/travelblog/{YYYY-MM}` | A month's posts on one page (e.g. `/travelblog/2001-08`), under `###` date headings; countries shown as flag + name; prev/next month nav. Heading is the month alone — the route back to `/travelblog` is the About sub-nav, rendered above the `h-entry` so its links stay out of the microformat and the Pagefind index |
